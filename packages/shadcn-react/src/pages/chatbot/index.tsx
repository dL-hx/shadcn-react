...
export default function Chatbot() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await streamBotResponse(userInput, onStreamUpdate);
    } catch (error) {
...
async function streamBotResponse(userInput: string, onStreamUpdate: (update: StreamUpdate) => void) {
  await callMcpChat(userInput, onStreamUpdate);
}

async function callMcpChat(userInput: string, onStreamUpdate: (update: StreamUpdate) => void) {
  const abortController = new AbortController();

  try {
    console.log('Calling mcp-chat with input:', userInput);
    const response = await fetch('http://localhost:3500/chat', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ userContent: userInput }),
